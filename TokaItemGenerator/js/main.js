$(function () {
  ...
  $(document).on("click", ".tab_controls_bar_tab li", function () {
    if (
      is_components_food_enable &&
      $(this).parents(".tab_controls").hasClass("components_food_effects")
    ) {
  ...
  $(".tab_controls_addTab").on("click", function () {
    if (is_components_food_enable && $(this).hasClass("components_food_effects")) {
      add_tab($(this).prev());
    }
    onChangedJSON();
  });
  ...
  $(document).on("click", ".components_food_remove_effect_delete", function () {
    ...
    remove_effect.hide(150, function () {
  ...
  function add_author(name) {
    ...
    remove_effect_list_child.append(
      $("<span>").addClass("components_food_remove_effect_delete").text("×")
    );
  ...
  function checkJSONError() {
    ...
    if (Item_ID == "") {
      ...
      addIssue(
        "error",
        '[Description:identifier] アイテムIDが空です。アイテムIDを入力してください。アイテムIDは"名前空間:アイテムID"を入力してください。'
      );
      error_num++;
      ...
      addIssue(
        "error",
        '[Description:identifier] アイテムIDが ":" で区切られていません。"名前空間:アイテムID"の形式になっている必要があります。'
      );
      error_num++;
      ...
      addIssue(
        "error",
        '[Description:identifier] アイテムIDがありません。"名前空間:アイテムID"を入力してください。'
      );
      error_num++;
    }
    return error_num;
    ...
    switch (format_version) {
      ...
      case "1.12.0":
        ...
        if (components["minecraft:item_light_absorption"].value != null) {
          $("#components_Item_light_absorption").val(
            components["minecraft:item_light_absorption"].value
          );
        }
        if (components["minecraft:item_light_emission"].emission != null) {
          $("#components_Item_light_emission").val(
            components["minecraft:item_light_emission"].emission
          );
        }
        break;
      default:
  ...
  function exportJSON() {
    ...
    if ($("#components_foil").is(":checked"))
      components["minecraft:foil"] = $("#components_foil").is(":checked");
    if ($("#components_hand_equipped").is(":checked"))
    ...
    if ($("#components_stacked_by_data").is(":checked") != "")
    ...
    if (is_components_food_enable) {
      ...
      if (
        ...
        Number(
          tab_content.find("#components_food_effects_duration").val() != 0 &&
        ...
        for (let i = 0; i < $(".components_food_effects.tab_controls_bar_tab li").length; i++) {
          ...
          tab_content = $(
            "div.components_food_effects.tab_content_list > div:nth-child(" + child_num + ")"
          );
          food.effects[i].name = tab_content.find("#components_food_effects_name").val();
          food.effects[i].chance = Number(
            tab_content.find("#components_food_effects_chance").val()
          );
          food.effects[i].duration = Number(
            tab_content.find("#components_food_effects_duration").val()
          );
          food.effects[i].amplifier = Number(
            tab_content.find("#components_food_effects_amplifier").val()
          );
        }
      }
      if ($("div.components_food_remove_effect_list > div").length > 0) {
        ...
        for (let i = 1; i <= $("div.components_food_remove_effect_list > div").length; i++) {
          food.remove_effect.push(
            $(
              "div.components_food_remove_effect_list > div:nth-child(" + i + ") > span.name"
            ).text()
          );
        }
      }
    ...
    if (is_components_camera_enable) {
      ...
      camera.black_bars_screen_ratio = parseFloat(
        $("#components_camera_black_bars_screen_ratio").val()
      );
